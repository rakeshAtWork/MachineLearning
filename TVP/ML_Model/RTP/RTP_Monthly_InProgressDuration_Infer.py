...
def find_months(start_date,end_date):
    ...
    # Extract Year and month number from the 'Date' column
    df['Year'] = df['Date'].dt.year
    df['month'] = df['Date'].dt.month

    # Get unique pairs of Year and month number
...
def ipd_infer(df, start_date='2023-01-01', end_date='2023-02-28', model_dir_path='.'):
    ...
    output_df = pd.DataFrame(columns=['Year', 'Month', 'Ticket Type', 'Priority', 'Category', 'Subcategory', 'Predicted Time'])
...
if __name__ == '__main__':
    ...
    print(df.head())
    df.to_csv('IPDP_Monthly_.csv', index=False)
